...
function Row({ row, toDateDetails, selectedTrackDate }) {
				...
				<TableCell align="right">
					<div className="track-name-artist">
						...
						<div className="track-details">
							<div className="track-name" onClick = {() => history.push(`/trackdetails/${row.rank}/${selectedTrackDate ? selectedTrackDate : toDateDetails}`) }role="presentation" >{row.track_name}</div>
...
const TrackTableList = ({ _trackTableList, trackDetails, fromDateDetails, toDateDetails, countrySelected, selectedTrackDate }) => {
	...
	useEffect(() => {
		if (fromDateDetails && toDateDetails && countrySelected) {
			_trackTableList(fromDateDetails, toDateDetails, countrySelected);
		}
				...
				<Table aria-label="collapsible table">
					...
					<TableBody>
						{trackDetails.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
							.map((row) => (
								<Row key={row.id} row={row} toDateDetails={toDateDetails} selectedTrackDate={selectedTrackDate}/>
							))}
					</TableBody>
				</Table>
...
const mapStateToProps = ({ Home, Common }) => {
	...
	const { trackDetails, countrySelected, labelSelected } = Home;

	return { trackDetails, countrySelected, labelSelected, fromDateDetails, toDateDetails, };
};

export default connect(mapStateToProps, mapDispatchToProps)(TrackTableList);
