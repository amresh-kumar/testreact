...
import { trackList } from "../../redux/actions/Home";
...
const CollapsibleTable = ({ _trackList, trackDetails, countrySelected, fromDateDetails, toDateDetails, selectedTrackDate}) => {
	...
	useEffect(() => {
		if (fromDateDetails && toDateDetails && countrySelected) {
			_trackList(fromDateDetails, toDateDetails, countrySelected);
		}
...
const mapStateToProps = ({ Home, Common }) => {
	...
	const { trackDetails, countrySelected } = Home;
	return { trackDetails, countrySelected, fromDateDetails, toDateDetails };
};
